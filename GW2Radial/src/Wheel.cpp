...
Wheel::Wheel(uint bgResourceId, uint inkResourceId, std::string nickname, std::string displayName, IDirect3DDevice9 * dev)
	: nickname_(std::move(nickname)), displayName_(std::move(displayName)),
	  ...
	  centerFavoriteOption_(u8"喜好选择", "center_favorite", "wheel_" + nickname_),
	  ...
	  noHoldOption_(u8"鼠标左键松开的情况下激活第一个盘旋选项", "no_hold", "wheel_" + nickname_, false)
{
...
void Wheel::DrawMenu()
{
	...
	ImGui::TextUnformatted(u8"跟游戏内按键设置必须一样:");
	...
	if(!noHoldOption_.value())
	{
		...
		ImGui::SameLine();
		ImGuiConfigurationWrapper(rb, u8"自定义", centerBehaviorOption_, int(CenterBehavior::FAVORITE));
